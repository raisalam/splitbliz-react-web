...
type QuickActionsProps = {
  groups: Array<{ name: string; emoji: string }>;
  allActions: ActionItem[];
  visibleActions: ActionItem[];
  hiddenCount: number;
  onShowAll: () => void;
  onAction: (id: string, type: ActionItem['type'], action: 'approve' | 'reject') => void;
};
              ...
              {visibleActions.map((action) => (
                ...
                >
                  ...
                  <div className="flex items-start sm:items-center gap-3 pl-2">
                    {(() => {
                      const grp = groups.find(g => g.name === action.groupName);
                      return <GroupAvatar name={action.groupName} emoji={grp?.emoji || 'ðŸ‘¥'} size="sm" />;
                    })()}
                    ...
                    <div>
                      <p className="text-sm text-slate-700 dark:text-slate-300">
                        ...
                        {action.type === 'SETTLEMENT' ? (
                          <> requested a settlement of <span className="font-semibold text-slate-900 dark:text-white">{action.currencyCode === 'INR' ? 'â‚¹' : '$'}{action.amount}</span> in <span className="font-semibold text-slate-900 dark:text-white">{action.groupName}</span></>
                        ) : (
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto pl-2 sm:pl-0 mt-2 sm:mt-0">
                    <button
                      onClick={() => onAction(action.id, action.type, 'reject')}
                    ...
                    <button
                      onClick={() => onAction(action.id, action.type, 'approve')}
